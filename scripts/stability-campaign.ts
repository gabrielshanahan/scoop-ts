 ...
 *
 * A run only counts as green at 200 passed / 0 failed (195 ported + 5 port-added regressions). If a run fails while the shared container
...
try {
    ...
    while (run <= totalRuns) {
        ...
        const ok = result.status === 0 && fail === "0" && pass === "200"
